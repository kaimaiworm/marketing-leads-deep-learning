...
input_folder         = "{}\\dataset\\input".format(path)
output_folder        = "{}\\dataset\\output".format(path)
...
# Convert type "nan" in ad_data to string
...
# Lead dictionary
leads_dict = {}
leads_dict["manual"] = leads_raw.loc[(leads_raw["Type"] == "Manual") & (leads_raw["DateCreated"] >= "2020-04-04") & (leads_raw["DateCreated"] <= "2023-04-30"), ["DateCreated", "NextDayLeads"]]
leads_dict["website"] = leads_raw.loc[(leads_raw["Type"] == "Website") & (leads_raw["DateCreated"] >= "2020-04-04") & (leads_raw["DateCreated"] <= "2023-04-30"), ["DateCreated", "NextDayLeads"]]
...
for col in macro_raw.columns:
    macro_dict[col] = macro_raw.loc[(macro_raw["Date"] >= "2020-04-04") & (macro_raw["Date"] <= "2023-04-30"), ["Date", col]] 
...
for col in ["Spend", "Impressions", "Clicks"]:
    ads_dict[col] = ads_all.loc[(ads_raw["Date"] >= "2020-04-04") & (ads_all["Date"] <= "2023-04-30"), ["Date", col]] 
...
for col in ifo_raw.columns:
    ifo_dict[col] = ifo_raw.loc[(ifo_raw["Date"] >= "2020-04-01") & (ifo_raw["Date"] <= "2023-04-30"), ["Date", col]] 
...
for col in ["Visits", "TimeSpent"]:
    website_dict[col] = website_raw.loc[(website_raw["Date"] >= "2020-04-04") & (website_raw["Date"] <= "2023-04-30"), ["Date", col]] 
...
# No data leakage in this imputation step
series = pd.date_range("2020-05-01", "2023-05-01", freq = "MS")

for key in ifo_dict.keys():
    # retrieve values from dict and set all values for april 2020, need to do this seperately because data does not start in first of april
    df_all.loc[(df_all["Date"] < "2020-05-01"), [key]] = np.array(ifo_dict[key].loc[(ifo_dict[key]["Date"] == "2020-04-01"), [key]]) 
    for i in range(0, len(series)):
        if i == len(series) -1 : 
            break #break one before series end, otherwise error because series it too long
        #set rest of each month to value of first day of month
...
# Split dataset into test, validation and test, use March 2023 as validation and April 2023 as test data 
df_train = df_all.loc[df_all.index < "2023-03-01"].copy()
df_val = df_all.loc[(df_all.index >= "2023-03-01") & (df_all.index < "2023-04-01")].copy()
df_test = df_all.loc[(df_all.index >= "2023-04-01")].copy()
...
#### Plot all variables for EDA (.show() uncommented because otherwise 20+ plots will open in browser)
...
df_val = remove_outlier(df_val, y = ["manual", "website"]) # no outlier detection for target variables in validation data
...
for df in [df_train, df_val, df_test]:
    ...
    for col in cols:
        ...
        # if data is at the end of frame, use next possible value
        ...
        # if data is on both sides of middle -> implies all NaNs, no interpolation possible, drop column (this does not happen but put it in for safety)
        else:
...
remove = ["Climate", "State", "Expectations"] #only 1 month lag for business climate variables, done seperately
for col in remove:
    cols.remove(col)
...
# Turn categorical weekday variables into binary dummies
enc = OneHotEncoder()
...
val_cat_enc= pd.DataFrame(enc.fit_transform(df_val[["weekday"]]).toarray(), columns = enc.get_feature_names_out(), index = df_val.index)
...
df_val = pd.concat([df_val, val_cat_enc], axis = 1)
df_test = pd.concat([df_test, test_cat_enc], axis = 1)

#Now drop categorical variable
df_all.drop(columns = ["weekday"], inplace = True)
df_train.drop(columns = ["weekday"], inplace = True)
df_val.drop(columns = ["weekday"], inplace = True)
df_test.drop(columns = ["weekday"], inplace = True)


##### Autocorrelation plots
#manual leads
plot_acf(df_all["manual"]) #-> shows pattern autocorrelation

# website leads
plot_acf(df_all["website"]) #-> shows significant autocorrelation, possible non-stationary

###### Since website leads are non-stationary and some predictors show trends, use differences for all variables
#No first diffs for dummies and near-static variables
cols = df_all.columns.tolist()
...
val_diff = df_all_diff.loc[df_all_diff.index.isin(df_val.index),]
test_diff = df_all_diff.loc[df_all_diff.index.isin(df_test.index),]

###### Drop NaNs in training data due to added lags
...
#### Detect outliers for differenced data 
# No outlier detection for weekday dummies and ifo index (gathered in "remove" variable) 
train_diff = remove_outlier(train_diff, y = remove)
val_diff = remove_outlier(val_diff, y = ["manual", "website"]+remove)
test_diff = remove_outlier(test_diff, y = ["manual", "website"]+remove)

# Cannot interpolate data that is at the beginning or end of data frame, fill with data of closest date 
for df in [train_diff, val_diff, test_diff]:
    cols = df.columns[df.isna().any()].tolist() # determine columns with NaNs
    if not cols: continue # Go to next dataframe if no NaNs
    for col in cols:
        rows = df.loc[df[col].isna(), col] # determine rows with NaNs for column
...
with pd.ExcelWriter("{}\\data_diff.xlsx".format(output_folder)) as writer:
    ...
    val_diff.to_excel(writer, sheet_name="val")
